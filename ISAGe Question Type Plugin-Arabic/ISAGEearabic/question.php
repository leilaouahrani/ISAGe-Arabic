...
/**
 * Represents an ISAGEearabic question.
 *
 * @copyright  
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
class qtype_ISAGEearabic_question extends question_graded_automatically {
...
/********************************************************************************************/
     ...
     */
        ...
        return get_string('pleaseenterananswer', 'qtype_ISAGEearabic');
    }
...
/**
    ...
    public function grade_response(array $response) {
     ...
     // Call to the external Grader 
      
        $donnee= self::safe_normalize($answer);    /* donnee = model answer */
        $quest= self::safe_normalize($question);   /* quest = question */
        $data =  str_replace(array("\r", "\n"), " ", self::safe_normalize($response['answer']));  /* data = student answer */
        
        $curlvar= curl_init(); //Initialize the cURL var
        curl_setopt($curlvar,CURLOPT_RETURNTRANSFER, 1); //Get the response from cURL
        curl_setopt($curlvar,CURLOPT_URL,'http://ISAGEearabic.pythonanywhere.com/?donnee='.self::checkInput($donnee).'&data='.self::checkInput($data).'&quest='.self::checkInput($quest)); //Set the Url
